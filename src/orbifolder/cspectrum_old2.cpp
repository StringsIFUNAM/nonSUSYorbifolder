...
CSpectrum::CSpectrum(const SConfig &Vacuum, const vector<SUSYMultiplet> &SUSYTypes, string Label, const vector<vector<unsigned> > *SectorsForAdditionalIdentifier)
{
  ...
  if (this->NumberOfSupersymmetry == 2)
  {
	...
	{
		cout << "\n  Warning in bool CSpectrum(...) : list of sorted spectra is empty.\n" << endl;
		ReOrder();
			//cout <<  "\n  Warning in bool CSpectrum(...) : ReOrder unsuccessful.\n";
		cout << endl << this->SortedSpectra.size() << endl;
	}*/
}
...
######   vacuum "Vacuum".                                                            ######
######################################################################################## */
CSpectrum::CSpectrum(const SConfig &Vacuum1, const SConfig &Vacuum2, const vector<SUSYMultiplet> &SUSYTypes, string Label, const vector<vector<unsigned> > *SectorsForAdditionalIdentifier)
{
	this->Label = Label;

	unsigned pos_of_U1Hypercharge = 0;
	this->UseSpecialU1 = false;
	if (Vacuum1.SymmetryGroup.observable_sector_U1s.size() == 1)
	{
		pos_of_U1Hypercharge = Vacuum1.SymmetryGroup.observable_sector_U1s[0];
		this->UseSpecialU1 = (Vacuum1.SymmetryGroup.U1s_AdditionalLabels[pos_of_U1Hypercharge] == "Y");
	}

	this->NumberOfSupersymmetry = Vacuum1.InvariantSupercharges.size();
  ...
  if (this->NumberOfSupersymmetry == 2)
  {
	...
	else
	{
		...
		const vector<CField> &Tachyons = Vacuum2.Fields;
		const size_t f2 = Tachyons.size();
		const size_t TachyonFree_ListLength = s1 = List.size();

		for (i = 0; i < f2; ++i)
		{
			const CField &Field = Tachyons[i];

			if (Field.Multiplet != Tachyon)
			{
				NewRep.Multiplet = Field.Multiplet;
				...
				for (j = TachyonFree_ListLength; j < s1; ++j)
				{
					SRepresentation &KnownRep = List[j];
...
bool CSpectrum::ReOrder()
{
	...
	{
		this->SortedSpectra.push_back(this->Spectrum);							// This is missing from revised CSpectrum routines!
	...
	if (TwoParts)
	{
		...
		if (this->OrderE8xE8(Columns[0], Columns[1]) == 2)
		{
			...
			//cout << "  save permutation: different E8xE8 ordering" << endl;
	...
	for (p = 0; p < NumberOfAllColumnPermutations; ++p)
	{
		...
		// begin: compare two adjacent columns
		if (true)
		{
		...
		// end: compare two adjacent columns
				...
				for (i = 0; i < NumberOfBlocks; ++i)
				{
					...
					if (s3 != 1)
					{
						...
						do
						{
							...
							NewSpectrum = this->Spectrum;
							this->InsertSortedColumnsIntoExistingSpectrum(NewSpectrum, NewColumns);
	...
	//cout << "  Number of inequivalent permutations: " << this->SortedSpectra.size() << endl;
...
bool RowCriterion2(const SRepresentation &rep1, const SRepresentation &rep2)
{
	...
	if ((rep1.Multiplicity == rep2.Multiplicity) && (rep1.Representation == rep2.Representation) && (fabs(rep1.U1Charge - rep2.U1Charge) < 0.0001))
		return true;
