...
/* ########################################################################################
######   bool CAnalyseModel::FindCouplings(...) const                                ######
######                                                                               ######
######   Version: 24.02.2012                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) VEVConfig         : contains the couplings                               ######
######   2) FieldLabels       : find all couplings involving the field labels of the ######
######                          list "FieldLabels"                                   ######
######   3) Result            : the list of couplings from "VEVConfig" involving     ######
######                          "FieldLabels"                                        ######
######   4) EffectiveCoupling : use effective couplings, i.e. first replace fields   ######
######                          by their VEVs for non-vanishing VEVs                 ######
######   output:                                                                     ######
######   return value         : finished succesfully?                                ######
###########################################################################################
######   description:                                                                ######
######   Find all couplings from "VEVConfig" involving the field labels "FieldLabels"######
######   and save the result to "Result".                                            ######
######################################################################################## */
bool CAnalyseModel::FindCouplings(const SConfig &VEVConfig, const vector<string> &FieldLabels, vector<YukawaCoupling> &Result, bool EffectiveCoupling) const
{
	// Set the precision
	...
	if (f1 != 0)
	{
		if (VEVConfig.use_Labels >= Fields[0].Labels.size())
		{
			cout << "\n  Warning in bool CAnalyseModel::FindCouplings(...): VEVConfig.use_Labels out of range. Return false." << endl;
			return false;
		}
	}

	vector<string> FieldLabels_withoutNumber;
	vector<string> FieldLabels_withNumber;
	vector<unsigned> FieldLabels_counter_withoutNumber;
	vector<unsigned> FieldLabels_counter_withNumber;

	vector<string>   *p2FieldLabels = NULL;
	vector<unsigned> *p2FieldLabels_counter = NULL;

	unsigned i = 0;
	unsigned j = 0;
	unsigned k = 0;

	bool rep_known = false;
	size_t t1 = 0;

	// begin: find the different field labels in "FieldLabels"
	const size_t s0 = FieldLabels.size();
	for (i = 0; i < s0; ++i)
	{
		const string &NextFieldLabel = FieldLabels[i];

		if (NextFieldLabel != "*")
		{
			// first, assume that the current representation is unknown
			rep_known = false;

			if (NextFieldLabel.find("_", 0) != string::npos)
			{
				p2FieldLabels         = &FieldLabels_withNumber;
				p2FieldLabels_counter = &FieldLabels_counter_withNumber;
			}
			else
			{
				p2FieldLabels         = &FieldLabels_withoutNumber;
				p2FieldLabels_counter = &FieldLabels_counter_withoutNumber;
			}

			t1 = p2FieldLabels->size();

			for (j = 0; !rep_known && (j < t1); ++j)
			{
				if (NextFieldLabel == p2FieldLabels->at(j))
				{
					rep_known = true;
					++p2FieldLabels_counter->at(j);
				}
			}

			if (!rep_known)
			{
				p2FieldLabels->push_back(NextFieldLabel);
				p2FieldLabels_counter->push_back(1);
			}
		}
	}
	// end: find the different field labels in "FieldLabels"

	YukawaCoupling Allowed_Coupling;
	unsigned current_order = 0;

	const size_t s1 = FieldLabels_withNumber.size();
	const size_t s2 = FieldLabels_withoutNumber.size();

	const vector<unsigned> empty_counter_withNumber(s1, 0);
	const vector<unsigned> empty_counter_withoutNumber(s2, 0);
	vector<unsigned> counter_withNumber;
	vector<unsigned> counter_withoutNumber;
	vector<bool>     FieldFound;

	vector<unsigned> FieldIndicesOfVEVs;

	bool ok = true;
	unsigned Index = 0;

	// run through the couplings
	const vector<YukawaCoupling> &FieldCouplings = VEVConfig.FieldCouplings;
	const size_t s3 = FieldCouplings.size();
	for (i = 0; i < s3; ++i)
	{
		vector<unsigned> FieldCoupling = FieldCouplings[i].FieldIndices;
		current_order = FieldCoupling.size();

		counter_withNumber    = empty_counter_withNumber;
		counter_withoutNumber = empty_counter_withoutNumber;

		FieldFound.assign(current_order, false);

		// begin: find fields with numbers
		for (j = 0; j < current_order; ++j)
		{
			const CField &tmp_Field = Fields[FieldCoupling[j]];

			string FieldLabel = tmp_Field.Labels[VEVConfig.use_Labels];
			FieldLabel += "_";
			ostringstream os;
			os << tmp_Field.Numbers[VEVConfig.use_Labels];
			FieldLabel += os.str();

			rep_known = false;
			for (k = 0; !rep_known && (k < s1); ++k)
			{
				if (FieldLabels_withNumber[k] == FieldLabel)
				{
					++counter_withNumber[k];
					rep_known = true;
					FieldFound[j] = true;
				}
			}
		}
		// end: find fields with numbers

		ok = true;
		for (j = 0; ok && (j < s1); ++j)
		{
			if (counter_withNumber[j] < FieldLabels_counter_withNumber[j])
				ok = false;
		}

		if (ok)
		{
			// begin: find fields without numbers
			for (j = 0; j < current_order; ++j)
			{
				if (!FieldFound[j])
				{
					...
					rep_known = false;
					for (k = 0; !rep_known && (k < s2); ++k)
					{
						if (FieldLabels_withoutNumber[k] == FieldLabel)
						{
							++counter_withoutNumber[k];
							rep_known = true;
							FieldFound[j] = true;
						}
					}
			...
			// end: find fields without numbers

			ok = true;
			for (j = 0; ok && (j < s2); ++j)
			{
				if (counter_withoutNumber[j] < FieldLabels_counter_withoutNumber[j])
					ok = false;
			}

			if (ok)
			{
				Allowed_Coupling.FieldIndices.clear();
				Allowed_Coupling.CouplingStrength = FieldCouplings[i].CouplingStrength;

				for (j = 0; j < current_order; ++j)
				{
					Index = FieldCoupling[j];
					Allowed_Coupling.FieldIndices.push_back(Index);
				}

				if (EffectiveCoupling)
				{
					for (j = 0; j < current_order; ++j)
					{
						const CField &tmp_Field = Fields[FieldCoupling[j]];
						if (!FieldFound[j] && (fabs(tmp_Field.VEVs.GetLength()) < prec))
							ok = false;
					}
				}
				if (ok)
				{
					Allowed_Coupling.LabelOfModuli   = FieldCouplings[i].LabelOfModuli;
					Allowed_Coupling.ExponentsOfEtas = FieldCouplings[i].ExponentsOfEtas;
					Result.push_back(Allowed_Coupling);
				}
			}
		}
	}
...
/* ########################################################################################
######   AccidentalU1Charges_Find(SConfig &VEVConfig, ...) const                     ######
######                                                                               ######
######   Version: 18.05.2011                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) VEVConfig                     : contains the couplings and the fields    ######
######   2) FieldIndicesWithZeroAccCharge : indices of those fields that shall have  ######
######                                      zero charge under the accidental U(1)    ######
######   3) Print                         : if "info" is true print the output to    ######
######                                      this CPrint object                       ######
######   4) info                          : print info?                              ######
######   output:                                                                     ######
######   return value                     : finished succesfully?                    ######
###########################################################################################
######   description:                                                                ######
######   Identifies accidental U(1) symmetries of the superpotential from "VEVConfig"######
######   and stores the result there.                                                ######
######################################################################################## */
bool CAnalyseModel::AccidentalU1Charges_Find(SConfig &VEVConfig, const vector<unsigned> &FieldIndicesWithZeroAccCharge, CPrint &Print, bool info) const
{
	vector<CField>               &Fields         = VEVConfig.Fields;
	const vector<YukawaCoupling> &FieldCouplings = VEVConfig.FieldCouplings;

	const size_t c1 = FieldCouplings.size();
	if (c1 == 0)
	{
		(*Print.out) << "\n  " << Print.cbegin << "Cannot find accidental U(1) symmetries because the superpotential is empty." << Print.cend << "\n" << endl;
		return false;
	}
	...
	const size_t f1 = Fields.size();
	vector<unsigned> Index2LeftChiralIndex(f1, 0);
	vector<unsigned> LeftChiralIndex2Index;
	for (i = 0; i < f1; ++i)
	{
		if (Fields[i].Multiplet == LeftChiral)
		{
			LeftChiralIndex2Index.push_back(i);
			Index2LeftChiralIndex[i] = j;
			++j;
		}
	}

	const size_t f2 = LeftChiralIndex2Index.size();
	if (f2 == 0)
	{
		(*Print.out) << "\n  " << Print.cbegin << "Cannot find accidental U(1) symmetries because there are no left-chiral fields in this vacuum." << Print.cend << "\n" << endl;
		return false;
	}

	if (info)
	{
		(*Print.out) << "  " << Print.cbegin << "Find accidental U(1)s: number of fields:          " << f2 << Print.cend << "\n";
		(*Print.out) << "  " << Print.cbegin << "Find accidental U(1)s: number of couplings:       " << c1 << Print.cend << "\n";
	}

	size_t s1 = 0;

	// begin: create the superpotential-matrix A
	vector<double> tmp_dline(f2,0.0);
	vector<vector<double> > MatrixA(c1, tmp_dline);

	for (i = 0; i < c1; ++i)
	{
		const vector<unsigned> &FieldIndices = FieldCouplings[i].FieldIndices;
		s1 = FieldIndices.size();
		for (j = 0; j < s1; ++j)
			MatrixA[i][Index2LeftChiralIndex[FieldIndices[j]]] += 1.0;
	}
	// end: create the superpotential-matrix A

	// begin: add artificial couplings in order to set accidental charges of some fields to zero
	if (FieldIndicesWithZeroAccCharge.size() != 0)
	{
		for (i = 0; i < f2; ++i)
		{
			// if the i-th left-chiral field "F" shall have zero accidental charge
			if (find(FieldIndicesWithZeroAccCharge.begin(), FieldIndicesWithZeroAccCharge.end(), LeftChiralIndex2Index[i]) != FieldIndicesWithZeroAccCharge.end())
			{
				// add a linear coupling "F" to the superpotential-matrix
				tmp_dline.assign(f2, 0.0);
				tmp_dline[i] = 1;
				MatrixA.push_back(tmp_dline);
			}
		}
	}
	// end: add artificial couplings in order to set accidental charges of some fields to zero

	// make the superpotential-matrix A quadratic
	vector<vector<double> > MatrixdS = transpose(MatrixA) * MatrixA;

	// begin: convert the quadratic superpotential-matrix to CHugeInt
	vector<rational<CHugeInt> > tmp_line(f2, rational<CHugeInt>(CHugeInt(0), CHugeInt(1))); //vector<rational<CHugeInt> > tmp_line(f2, rational<CHugeInt>(0,1));
	vector<vector<rational<CHugeInt> > > MatrixS(f2, tmp_line);

	for (i = 0; i < f2; ++i)
	{
		for (j = 0; j < f2; ++j)
			MatrixS[i][j] = (CHugeInt)((int)MatrixdS[i][j]);
	}
	// end: convert the quadratic superpotential-matrix to CHugeInt

	vector<vector<rational<CHugeInt> > > Basis;
	CLinAlg<CHugeInt> LA;
	LA.FindKernel(MatrixS, Basis);

	const size_t Number_Of_AccidentalSymmetries = Basis.size();

	if (info)
		(*Print.out) << "  " << Print.cbegin << "Find accidental U(1)s: number of U(1) symmetries: " << Number_Of_AccidentalSymmetries << Print.cend << "\n";

	// begin: check invariance of the superpotential under accidental global U1 symmetry
	const rational<CHugeInt> Zero;
	rational<CHugeInt> U1_sum = Zero;

	for (i = 0; i < Number_Of_AccidentalSymmetries; ++i)
	{
		const vector<rational<CHugeInt> > &Charges = Basis[i];
		...
		for (j = 0; j < c1; ++j)
		{
			...
			s1 = FieldIndices.size();
			for (k = 0; k < s1; ++k)
				U1_sum += Charges[Index2LeftChiralIndex[FieldIndices[k]]];

			if (U1_sum != Zero)
			{
				(*Print.out) << "  " << Print.cbegin << "Warning: Superpotential is not invariant under accidental global U(1) symmetry! U1_sum = " << U1_sum << Print.cend << "\n" << endl;
				return false;
			}
		}
	}
	...
	for (i = 0; i < f2; ++i)
	{
		for (j = 0; j < f2; ++j)
		{
			for (k = 0; k < Number_Of_AccidentalSymmetries; ++k)
				MatrixB[i][j] += Basis[k][i] * Basis[k][j];
		}
	}
	vector<vector<rational<CHugeInt> > > OrthogonalAccidentalSymmetry;
	LA.FindKernel(MatrixB, OrthogonalAccidentalSymmetry);

	if (info)
		(*Print.out) << "  " << Print.cbegin << "Find accidental U(1)s: number of broken U(1)s:    " << OrthogonalAccidentalSymmetry.size() << Print.cend << "\n";

	const size_t U1Size   = VEVConfig.SymmetryGroup.GaugeGroup.u1directions.size();
	const size_t OAccSize = OrthogonalAccidentalSymmetry.size();

	if (Number_Of_AccidentalSymmetries + OAccSize != f2)
	{
		(*Print.out) << "  " << Print.cbegin << "Warning: Cannot find the orthogonal symmetry space." << Print.cend << "\n" << endl;
		return false;
	}

	vector<rational<CHugeInt> > GaugeSymmetry(f2);
	for (i = 0; i < U1Size; ++i)
	{
		for (j = 0; j < f2; ++j)
			GaugeSymmetry[j] = D2HugeInt(Fields[LeftChiralIndex2Index[j]].U1Charges[i]);

		OrthogonalAccidentalSymmetry.push_back(GaugeSymmetry);
	}

	const size_t OAccSize2 = OrthogonalAccidentalSymmetry.size();

	MatrixB.clear();
	MatrixB.assign(f2, LineFSize);
	for (i = 0; i < f2; ++i)
	{
		for (j = 0; j < f2; ++j)
		{
			for (k = 0; k < OAccSize2; ++k)
				MatrixB[i][j] += OrthogonalAccidentalSymmetry[k][i] * OrthogonalAccidentalSymmetry[k][j];
		}
	}

	vector<vector<rational<CHugeInt> > > PureAccidentalSymmetry;
	LA.FindKernel(MatrixB, PureAccidentalSymmetry);

	const size_t PureAccSize = PureAccidentalSymmetry.size();

	if ((PureAccSize + OAccSize2 != f2) || (PureAccSize != Number_Of_AccidentalSymmetries - U1Size))
	{
		(*Print.out) << "  " << Print.cbegin << "Warning: Cannot find the purely accidental symmetry space." << Print.cend << "\n" << endl;
		return false;
	}

	if (info)
		(*Print.out) << "  " << Print.cbegin << "Find accidental U(1)s: number of accidental U(1)s " << PureAccSize << Print.cend << endl;
	// end: separate the gauge symmetires

	for (j = 0; j < f1; ++j)
		Fields[j].AccU1Charges.clear();

	for (i = 0; i < PureAccSize; ++i)
	{
		const vector<rational<CHugeInt> > &Charges = PureAccidentalSymmetry[i];
		for (j = 0; j < f2; ++j)
			Fields[LeftChiralIndex2Index[j]].AccU1Charges.push_back(Charges[j]);
	}
	...
	for (i = 0; i < PureAccSize; ++i)
	{
		...
		for (j = 0; j < c1; ++j)
		{
			...
			if (U1_sum != Zero)
			{
				(*Print.out) << "  " << Print.cbegin << "Warning: Superpotential is not invariant under (purely) accidental global U(1) symmetry! U1_sum = " << U1_sum << Print.cend << "\n" << endl;
				return false;
			}
		}
	}
...
/* ########################################################################################
######   FTerms_Compute(SConfig &VEVConfig, unsigned upto_Order_inW) const           ######
######                                                                               ######
######   Version: 25.03.2011                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) VEVConfig      : contains the superpotential W and the fields            ######
######   2) upto_Order_inW : use terms in the superpotential W only upto the order   ######
######                       "upto_Order_inW"                                        ######
######   output:                                                                     ######
######   -                                                                           ######
###########################################################################################
######   description:                                                                ######
######   Computes the F terms for the fields and the couplings of "VEVConfig", i.e.  ######
######   F_i = \partial W /  \partial\phi_i .                                        ######
######################################################################################## */
void CAnalyseModel::FTerms_Compute(SConfig &VEVConfig, unsigned upto_Order_inW) const
{
	if (VEVConfig.FTerms.size() != 0)
	{
		cout << "\n  Warning in void CAnalyseModel::FTerms_Compute(...) const: F-terms not zero. Now cleared!" << endl;
		VEVConfig.FTerms.clear();
	}
	...
	vector<YukawaCoupling> F_Term;
	YukawaCoupling Standard_F_Term, tmp_F_Term;
	Standard_F_Term.FieldIndices.clear();
	Standard_F_Term.CouplingStrength = 1.0;

	unsigned number_f_i = 0;

	// run through the fields f_i
	for (i = 0; i < f1; ++i)
	{
		if (Fields[i].Multiplet == LeftChiral)
		{
			...
			// run through the terms of the superpotential and compute F_i = d W / d f_i
			for (j = 0; j < s1; ++j)
			{
				const YukawaCoupling &FieldCoupling = FieldCouplings[j];
				s2 = FieldCoupling.FieldIndices.size();
				if (s2 <= upto_Order_inW)
				{
					tmp_F_Term = Standard_F_Term;

					number_f_i = 0;

					// run through the current coupling
					for (k = 0; k < s2; ++k)
					{
						// count how often field f_i appears in the current coupling
						if (FieldCoupling.FieldIndices[k] == i)
							++number_f_i;
						else
							tmp_F_Term.FieldIndices.push_back(FieldCoupling.FieldIndices[k]);
					}
					if (number_f_i != 0)
					{
						tmp_F_Term.CouplingStrength = FieldCoupling.CouplingStrength * number_f_i;

						if (number_f_i > 1)
							tmp_F_Term.FieldIndices.insert(tmp_F_Term.FieldIndices.begin(), number_f_i-1, i);

						if (tmp_F_Term.FieldIndices.size() > 0)
							F_Term.push_back(tmp_F_Term);
					}
				}
			}
			VEVConfig.FTerms.push_back(F_Term);
		}
	}
}
