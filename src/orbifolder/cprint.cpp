		...
		for (unsigned j = 0; j < 4; ++j)
		{
			if (DiscreteSymmetry.ChargeOperator[j] != 0)
			{
				...
				if ( DiscreteSymmetry.ChargeOperator[j] == 1)
					...
					(*this->out) <<  DiscreteSymmetry.ChargeOperator[j] << "R_" << j;
			}
		}
		(*this->out) << ", R(W) = " << DiscreteSymmetry.SuperpotentialCharge;
	}
	else
	{
		(*this->out) << ", charge: ";
...
bool CPrint::PrintFTerms(const SConfig &VEVConfig) const
{
	...
	const unsigned print_max_terms = 99;
	...
	{
		if (Fields[f].Multiplet == LeftFermi)
		{
			if (i >= s1)
			{
				//cout << "\n  Warning in bool CPrint::PrintFTerms(..) const: index out of range. Return false." << endl;
				cout << "\n  Warning in bool CPrint::PrintFTerms(..) const: Not adjusted for N=0 theory. Nothing to print." << endl; //hacking here!!!
				return false;
			}
