...
/* ########################################################################################
######   PrintDiscreteSymmetry(const SDiscreteSymmetry &DiscreteSymmetry) const      ######
######                                                                               ######
######   Version: 16.05.2013                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) DiscreteSymmetry : the SDiscreteSymmetry object containing the symmetry  ######
######                         to be printed                                         ######
######   output:                                                                     ######
######   return value        : printed succesfully?                                  ######
######################################################################################## */
bool CPrint::PrintDiscreteSymmetry(const SDiscreteSymmetry &DiscreteSymmetry) const
{
  (*this->out) << "Z_" << DiscreteSymmetry.Order;

  bool first_term_printed = false;
  if ((DiscreteSymmetry.ChargeOperator.size() == 4) || (DiscreteSymmetry.ChargeOperator.size() == 11) || (DiscreteSymmetry.ChargeOperator.size() == 12) || (DiscreteSymmetry.ChargeOperator.size() == 18))
  {
    if (DiscreteSymmetry.SuperpotentialCharge != 0)
      (*this->out) << "^R";
    (*this->out) << ", charge: ";
    for (unsigned j = 0; j < 4; ++j)
    {
      if (DiscreteSymmetry.ChargeOperator[j] != 0)
      {
        if (DiscreteSymmetry.ChargeOperator[j] < 0)
          (*this->out) << " - ";
        ...
        if (abs(DiscreteSymmetry.ChargeOperator[j]) != 1)
        {
          this->PrintRational(abs(DiscreteSymmetry.ChargeOperator[j]), false);
        (*this->out) << " ";
        }
        (*this->out) << "R_" << j;
      }
      ...
      for (unsigned j = 4; j < 12; ++j)
      {
        if (DiscreteSymmetry.ChargeOperator[j] != 0)
        {
          ...
          if (j == 5)
            ...
            (*this->out) << "gamma_" << j-5 << " n_" << j-5;
        }
      }
    }
    (*this->out) << ", R(W) = ";
    this->PrintRational(DiscreteSymmetry.SuperpotentialCharge, false);
  }
  else
  {
    (*this->out) << ", charge: ";
    for (unsigned j = 0; j < 8; ++j)
    {
      if (DiscreteSymmetry.ChargeOperator[j] != 0)
      {
        ...
        if (j == 0)
        {
          if (DiscreteSymmetry.ChargeOperator[j] == 1)
            (*this->out) << "k";
          else
            (*this->out) << DiscreteSymmetry.ChargeOperator[j] << "k";
        }
        else
        if (j == 1)
        {
          if (DiscreteSymmetry.ChargeOperator[j] == 1)
            ...
            (*this->out) << DiscreteSymmetry.ChargeOperator[j] << "l";
        }
        else
        {
          if (DiscreteSymmetry.ChargeOperator[j] == 1)
            ...
            (*this->out) << DiscreteSymmetry.ChargeOperator[j] << "n_" << j-1;
        }
      }
    }
    (*this->out) << ", generator: ";
    this->PrintSGElement(DiscreteSymmetry.SG_SymmetryGenerator, true);
  }
  (*this->out) << flush;
  return true;
}
...
/* ########################################################################################
######   PrintModularSymmetry(const SModularSymmetry &ModularSymmetry) const         ######
######                                                                               ######
######   Version: 12.10.2011                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) ModularSymmetry : the SModularSymmetry object containing the symmetry    ######
######                        to be printed                                          ######
######   output:                                                                     ######
######   return value       : printed succesfully?                                   ######
######################################################################################## */
bool CPrint::PrintModularSymmetry(const SModularSymmetry &ModularSymmetry) const
{
  if (ModularSymmetry.ChargeOperator.size() != 3)
  {
    cout << "\n  Warning in bool CPrint::PrintModularSymmetry(...) const: size of \"ChargeOperator\" is not three. Return false." << endl;
    return false;
  }

  (*this->out) << ModularSymmetry.Label << ", charge: n = q_sh_" << ModularSymmetry.Index << " (for q_sh_" << ModularSymmetry.Index << " = 0,-1)  or  n = ";
  PrintRational(ModularSymmetry.Const, false);
  (*this->out) << " ";

  if (ModularSymmetry.ChargeOperator[0] == 1)
    (*this->out) << "+";
  else
  if (ModularSymmetry.ChargeOperator[0] == -1)
    (*this->out) << "-";
  else
    (*this->out) << ModularSymmetry.ChargeOperator[0];
  (*this->out) << " q_sh_" << ModularSymmetry.Index << " ";
  
  if (ModularSymmetry.ChargeOperator[1] == 1)
    (*this->out) << "+";
  else
  if (ModularSymmetry.ChargeOperator[1] == -1)
    (*this->out) << "-";
  else
    (*this->out) << ModularSymmetry.ChargeOperator[1];
  (*this->out) << " N^" << ModularSymmetry.Index << " ";

  if (ModularSymmetry.ChargeOperator[2] == 1)
    (*this->out) << "+";
  else
  if (ModularSymmetry.ChargeOperator[2] == -1)
    (*this->out) << "-";
  else
    (*this->out) << ModularSymmetry.ChargeOperator[2];
  (*this->out) << " N^(-" << ModularSymmetry.Index << ") (else)" << flush;
  return true;
}
...
/* ########################################################################################
######   PrintDiscreteTorsion(const GDT_Parameters &DiscreteTorsion) const           ######
######                                                                               ######
######   Version: 07.07.2011                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) DiscreteTorsion : the parameters of (generalized) discrete torsion       ######
######   output:                                                                     ######
######   -                                                                           ######
######################################################################################## */
void CPrint::PrintDiscreteTorsion(const GDT_Parameters &DiscreteTorsion) const
{
  (*this->out) << "  a = " << DiscreteTorsion.a << this->endofset << "\n";
  (*this->out) << "  b = {" << DiscreteTorsion.b[0] << ", " << DiscreteTorsion.b[1] << ", " << DiscreteTorsion.b[2] << ", " << DiscreteTorsion.b[3] << ", " << DiscreteTorsion.b[4] << ", " << DiscreteTorsion.b[5] << "}" << this->endofset << "\n";
  (*this->out) << "  c = {" << DiscreteTorsion.c[0] << ", " << DiscreteTorsion.c[1] << ", " << DiscreteTorsion.c[2] << ", " << DiscreteTorsion.c[3] << ", " << DiscreteTorsion.c[4] << ", " << DiscreteTorsion.c[5] << "}" << this->endofset << "\n";
  (*this->out) << "  d = {\n";
  (*this->out) << "        {" << DiscreteTorsion.d[0][0] << ", " << DiscreteTorsion.d[0][1] << ", " << DiscreteTorsion.d[0][2] << ", " << DiscreteTorsion.d[0][3] << ", " << DiscreteTorsion.d[0][4] << ", " << DiscreteTorsion.d[0][5] << "},\n";
  (*this->out) << "        {" << DiscreteTorsion.d[1][0] << ", " << DiscreteTorsion.d[1][1] << ", " << DiscreteTorsion.d[1][2] << ", " << DiscreteTorsion.d[1][3] << ", " << DiscreteTorsion.d[1][4] << ", " << DiscreteTorsion.d[1][5] << "},\n";
  (*this->out) << "        {" << DiscreteTorsion.d[2][0] << ", " << DiscreteTorsion.d[2][1] << ", " << DiscreteTorsion.d[2][2] << ", " << DiscreteTorsion.d[2][3] << ", " << DiscreteTorsion.d[2][4] << ", " << DiscreteTorsion.d[2][5] << "},\n";
  (*this->out) << "        {" << DiscreteTorsion.d[3][0] << ", " << DiscreteTorsion.d[3][1] << ", " << DiscreteTorsion.d[3][2] << ", " << DiscreteTorsion.d[3][3] << ", " << DiscreteTorsion.d[3][4] << ", " << DiscreteTorsion.d[3][5] << "},\n";
  (*this->out) << "        {" << DiscreteTorsion.d[4][0] << ", " << DiscreteTorsion.d[4][1] << ", " << DiscreteTorsion.d[4][2] << ", " << DiscreteTorsion.d[4][3] << ", " << DiscreteTorsion.d[4][4] << ", " << DiscreteTorsion.d[4][5] << "},\n";
  (*this->out) << "        {" << DiscreteTorsion.d[5][0] << ", " << DiscreteTorsion.d[5][1] << ", " << DiscreteTorsion.d[5][2] << ", " << DiscreteTorsion.d[5][3] << ", " << DiscreteTorsion.d[5][4] << ", " << DiscreteTorsion.d[5][5] << "}\n";
  (*this->out) << "      }" << this->endofset << endl;
}
...
bool CPrint::PrintEigenvalues(const vector<COrbifoldGroupElement> &Centralizer, const vector<double> &Eigenvalues) const
{
  ...
  for (i = 0; i < s1; ++i)
  {
    ...
    (*this->out) << "(" << CElement.Get_k() << ", " << CElement.Get_l() << ") ("
...
/* ########################################################################################
######   PrintFTerms(const SConfig &VEVConfig) const                                 ######
######                                                                               ######
######   Version: 13.06.2013                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) VEVConfig : print the F terms of this vev-config                         ######
######   output:                                                                     ######
######   return value : printed succesfully?                                         ######
######################################################################################## */
bool CPrint::PrintFTerms(const SConfig &VEVConfig) const
{
  // Set the precision
  const double prec = 0.0001;

  const unsigned use_Labels = VEVConfig.use_Labels;
  const unsigned print_max_terms = 100;
  const unsigned terms_per_line = 5;
  
  vector<unsigned>::iterator pos;
  vector<unsigned> Known_FieldIndices;
  vector<unsigned> Known_counter;

  double F_term = 0.0;
  double vev = 1.0;
  bool print = true;
  bool firstTerm = true;
  unsigned counter_ZeroF    = 0;
  unsigned counter_NonZeroF = 0;
  ...
  const size_t s1 = VEVConfig.FTerms.size();
  size_t s2 = 0;
  size_t s3 = 0;

  (*this->out) << "\n";

  // run through the F-terms
  for (unsigned f = 0; f < f1; ++f)
  {
    if (Fields[f].Multiplet == LeftChiral)
    {
      if (i >= s1)
      {
        cout << "\n  Warning in bool CPrint::PrintFTerms(..) const: index out of range. Return false." << endl;
        return false;
      }

      firstTerm = true;
      const vector<YukawaCoupling> &F_i_Terms = VEVConfig.FTerms[i];
      ++i;
      s2 = F_i_Terms.size();
      ...
      for (j = 0; j < s2; ++j)
      {
        const YukawaCoupling &F_i_Term = F_i_Terms[j];
        vev = 1.0;
        s3 = F_i_Term.FieldIndices.size();
        for (k = 0; k < s3; ++k)
          vev *= Fields[F_i_Term.FieldIndices[k]].VEVs.GetLength();

        F_term += F_i_Term.CouplingStrength * vev;
      }
      // end: compute <F_i>

      // begin: if <F_i> != 0 print the result
      if (fabs(F_term) > prec)
      {
        ++counter_NonZeroF;

        (*this->out) << "  <F(";
        this->PrintLabel(Fields[f], use_Labels);
        (*this->out) << ")> = ";

        counter = 0;

        // run through the contributions to the current F-term
        for (j = 0; j < s2; ++j)
        {
          const YukawaCoupling &F_i_Term = F_i_Terms[j];
          s3 = F_i_Term.FieldIndices.size();

          // begin: does the current contribution vanish
          print = true;
          for (k = 0; print && (k < s3); ++k)
          {
            if (fabs(Fields[F_i_Term.FieldIndices[k]].VEVs.GetLength()) < prec)
              print = false;
          }
          // end: does the current contribution vanish
          ...
          if (print)
          {
            ...
            {
              if (counter < print_max_terms)
                (*this->out) << " + ";
            }

            // begin: print the current term
            if (counter < print_max_terms)
            {
              (*this->out) << F_i_Term.CouplingStrength << " <";
              for (k = 0; k < s3; ++k)
              {
                if (k != 0)
                  (*this->out) << " ";
                this->PrintLabel(Fields[F_i_Term.FieldIndices[k]], use_Labels);
              }
              (*this->out) << ">";
            }
            // end: print the current term

            // begin: make statistics
            for (k = 0; k < s3; ++k)
            {
              pos = find(Known_FieldIndices.begin(), Known_FieldIndices.end(), (unsigned)F_i_Term.FieldIndices[k]);
              if (pos == Known_FieldIndices.end())
              {
                Known_FieldIndices.push_back(F_i_Term.FieldIndices[k]);
                Known_counter.push_back(1);
              }
              else
                ++Known_counter[distance(Known_FieldIndices.begin(), pos)];
            }
            // end: make statistics
            ...
            if (counter % terms_per_line == 0)
              (*this->out) << "\n           ";
          }
        }
        (*this->out) << " = " << setw(5) << setprecision(2) << F_term << this->endofset << endl;
      }
      else
        ++counter_ZeroF;
      // end: if <F_i> != 0 print the result
    }
  }

  bool F_equals_zero = true;

  if (counter_NonZeroF == 0)
    (*this->out) << "  " << this->cbegin << "F = 0 in this vev-configuration." << this->cend << "\n";
  else
  {
    F_equals_zero = false;
    (*this->out) << "\n  " << this->cbegin << "No. of vanishing F-terms:     " << counter_ZeroF << this->cend << "\n  " << this->cbegin << "No. of non-vanishing F-terms: " << counter_NonZeroF << this->cend << "\n";

    (*this->out) << "\n  " << this->cbegin << "Statistics: how often do fields appear in F terms?" << this->cend << "\n";
    s2 = Known_FieldIndices.size();
    for (k = 0; k < s2; ++k)
    {
      (*this->out) << "  ";
      this->PrintLabel(Fields[Known_FieldIndices[k]], use_Labels);
      (*this->out) << "\t : " << Known_counter[k] << "\n";
    }
  }
  (*this->out) << endl;
  return F_equals_zero;
}
...
bool CPrint::PrintGaugeGroupFactor(const SConfig &VEVConfig, const unsigned factor) const
{
  ...
  {
    if (this->output_type == Tlatex)
      (*this->out) << "_\\text{" << VEVConfig.SymmetryGroup.GGs_AdditionalLabels[factor] << "}";
    else
...
void CPrint::PrintGaugeGroup(const SConfig &VEVConfig, bool print_selection) const
{
  ...
  else
  {
    ...
    if (u1 != 0)
    {
      ...
      if ((VEVConfig.SymmetryGroup.observable_sector_U1s[0] == 0) && VEVConfig.SymmetryGroup.IsFirstU1Anomalous)
...
######   -                                                                           ######
######################################################################################## */
void CPrint::PrintIdentifiedWilsonLines(const vector<vector<int> > &IdentifiedWilsonLines) const
{
  ...
  for (unsigned i = 0; i < i1; ++i)
  {
    ...
    const vector<int> &tmp_IdWilsonLines = IdentifiedWilsonLines[i];
    (*this->out) << this->set_open;
    if (tmp_IdWilsonLines[0] > 0)
      (*this->out) << "W" << this->start_index << tmp_IdWilsonLines[0];
    else
      (*this->out) << "-W" << this->start_index << abs(tmp_IdWilsonLines[0]);

    const size_t i2 = tmp_IdWilsonLines.size();
    for (unsigned j = 1; j < i2; ++j)
    {
      if (tmp_IdWilsonLines[j] > 0)
        (*this->out) << " = W" << this->start_index << tmp_IdWilsonLines[j];
      else
        (*this->out) << " = -W" << this->start_index << abs(tmp_IdWilsonLines[j]);
    }
...
bool CPrint::PrintLeftMovers(const CFixedBrane &FixedBrane, const vector<CModedOscillator> &all_Oscillators, const COrbifoldGroupElement &constructing_Element) const
{
  ...
  (*this->out) << constructing_Element.SGElement.Get_k() << ", " << constructing_Element.SGElement.Get_l() << ") (";
  for (i = 0; i < 6; ++i)
  {
...
bool CPrint::PrintRightMovers(const CSector &Sector) const
{
  ...
  (*this->out) << "  sector (" << Sector.Get_k() << ", " << Sector.Get_l() << ")\n  local Twist = ";
...
bool CPrint::PrintLabel(const CField &Field, unsigned useLabel) const
{
  ...
  (*this->out) << this->prelabel;
  if (output_type == Tlatex)
  {
    if (Field.Labels[useLabel] == "l")
      (*this->out) << "\\ell";
    else
    if (Field.Labels[useLabel] == "bq")
      (*this->out) << "\\bar{q}";
    else
    if (Field.Labels[useLabel] == "bu")
      (*this->out) << "\\bar{u}";
    else
    if (Field.Labels[useLabel] == "bd")
      (*this->out) << "\\bar{d}";
    else
    if (Field.Labels[useLabel] == "be")
      (*this->out) << "\\bar{e}";
    else
    if (Field.Labels[useLabel] == "bl")
      (*this->out) << "\\bar{\\ell}";
    else
      (*this->out) << Field.Labels[useLabel];
  }
  else
    (*this->out) << Field.Labels[useLabel];

  (*this->out) << this->start_index;
  if (output_type == Tlatex)
...
void CPrint::PrintListOfCharges(const COrbifold &Orbifold, const SConfig &VEVConfig, const vector<unsigned> &FieldIndices, const string &LabelOfList) const
{
  ...
  for (i = 0; i < f1; ++i)
  {
    ...
    for (j = 0; j < s1; ++j)
    {
      ...
      this->PrintRational(tmp_Field.q_sh, Lattice);
      (*this->out) << this->separator << " ";
      ...
      if (print_Rcharges)
      {
        ...
        for (k = 0; k < ds2; ++k)
        {
          this->PrintRational(tmp_Field.GetDiscreteCharge(DiscreteRSymmetries[k], SpaceGroup, Orbifold.OrbifoldGroup));
          if (k < ds2-1)
            (*this->out) << this->separator;
        }
      ...
      if (print_localization)
      {
        ...
        for (k = 0; k < ds1; ++k)
        {
          this->PrintRational(tmp_Field.GetDiscreteCharge(DiscreteNonRSymmetries[k], SpaceGroup, Orbifold.OrbifoldGroup));
          if (k < ds1-1)
            (*this->out) << this->separator;
        }
        (*this->out) << this->vector_close << this->separator << " ";
...
bool CPrint::PrintMasslessRightMovers(const CSector &Sector) const
{
  ...
  (*this->out) << "  " << this->cbegin << "sector (" << Sector.Get_k() << ", " << Sector.Get_l() << ")" << this->cend << "\n";
...
/* ########################################################################################
######   PrintMassMatrix(const CMassMatrix &MassMatrix, ...) const                   ######
######                                                                               ######
######   Version: 04.04.2011                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) MassMatrix : CMassMatrix object to be printed                            ######
######   2) VEVConfig  : contains the fields (i.e. their labels)                     ######
######   3) max_order  : print fields explicitly up to this order                    ######
######   output:                                                                     ######
######   -                                                                           ######
######################################################################################## */
void CPrint::PrintMassMatrix(const CMassMatrix &MassMatrix, const SConfig &VEVConfig, unsigned max_order) const
{
  // Set the precision
  const double prec = 0.0001;
    
  const vector<CField> &Fields = VEVConfig.Fields;
  const vector<vector<vector<YukawaCoupling> > > &Matrix = MassMatrix.GetMatrix();
  
  const size_t s1 = Matrix.size();
  size_t s2 = 0;
  ...
  (*this->out) << "\n  mass matrix: ";
  this->PrintMassMatrixInfo(MassMatrix, VEVConfig);
  (*this->out) << endl;

  const bool tex = (output_type == Tlatex);
  // begin: tex table
  if (tex)
  {
    (*this->out) << "\n";
    ...
    (*this->out) << "\\left(\n";
    (*this->out) << "\\begin{array}{" << tabular_parameter << "}\n";
  }
  // end: tex table

  bool first_entry_printed = false;
  ...
  for (i = 0; i < s1; ++i)
  {
    const vector<vector<YukawaCoupling> > &row = Matrix[i];
    if (!tex)
      (*this->out) << "\t";
    ...
    for (j = 0; j < s2; ++j)
    {
      const vector<YukawaCoupling> &entry = row[j];
      const size_t number_of_couplings = entry.size();

      // begin: find minimal order of the current "entry"
      unsigned     minimal_order = 100;
      vector<bool> couplings_with_vev(number_of_couplings, false);
      unsigned     counter_couplings_with_vev = 0;

      // run through the couplings of current "entry" of the mass matrix
      for (k = 0; k < number_of_couplings; ++k)
      {
        const vector<unsigned> &coupling = entry[k].FieldIndices;
        current_order = coupling.size();

        //check, wether one state of current "coupling" has no vev
        bool no_vev_state_found = false;
        for (l = 0; !no_vev_state_found && (l < current_order); ++l)
        {
          if (fabs(Fields[coupling[l]].VEVs.GetLength()) < prec)
            no_vev_state_found = true;
        }
        if (!no_vev_state_found)
        {
          if (current_order < minimal_order)
            minimal_order = current_order;

          couplings_with_vev[k] = true;
          ++counter_couplings_with_vev;
        }
      }
      // end: find minimal order of the current "entry"

      first_entry_printed = false;

      if (counter_couplings_with_vev == 0)
        (*this->out) << "0";
      else
      {
        // begin: print s^order
        if (minimal_order > max_order)
        {
          if (minimal_order < 10)
          {
            if (minimal_order == 1)
        ...
        // begin: print vevs
        {
          // run through the couplings
          for (k = 0; k < number_of_couplings; ++k)
          {
            if (couplings_with_vev[k])
            {
              const vector<unsigned> &coupling = entry[k].FieldIndices;
              current_order = coupling.size();
              ...
              if (current_order <= max_order)
              {
                ...
                for (l = 0; l < current_order; ++l)
                {
                  (*this->out) << "<";
                  this->PrintLabel(Fields[coupling[l]], VEVConfig.use_Labels);
                  (*this->out) << ">";
                  
                  if (l+1 < current_order)
                    (*this->out) << " ";
                }
                // end: print coupling
              }
...
/* ########################################################################################
######   PrintMassMatrixInfo(const CMassMatrix &MassMatrix, ...) const               ######
######                                                                               ######
######   Version: 29.02.2011                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
...
######   -                                                                           ######
######################################################################################## */
void CPrint::PrintMassMatrixInfo(const CMassMatrix &MassMatrix, const SConfig &VEVConfig) const
{
  const vector<CField> &Fields  = VEVConfig.Fields;
  const size_t f1 = Fields.size();

  // count the number of "Label_Row" and "Label_Column"
  const size_t s1 = MassMatrix.FieldIndices_Row.size();
  const size_t s2 = MassMatrix.FieldIndices_Column.size();

  // begin: mass matrix for all states with labels "Label_Row" and "Label_Column"?
  // if yes -> normal = true
  // if no  -> normal = false
  bool normal = true;
  ...
  unsigned i = 0;
  string tmp = "";
  unsigned counterLabel_Row = 0;
  unsigned counterLabel_Column = 0;
  for (i = 0; i < f1; ++i)
  {
    tmp = Fields[i].Labels[VEVConfig.use_Labels];
    if (tmp == MassMatrix.Label_Row)
      ++counterLabel_Row;
    if (tmp == MassMatrix.Label_Column)
      ++counterLabel_Column;
  }

  if ((s1 != counterLabel_Row) || (s2 != counterLabel_Column))
    normal = false;
  // end: mass matrix for all states with labels "Label_Row" and "Label_Column"?
  ...
  {
    if (tex)
      (*this->out) << MassMatrix.Label_Row << "_i M_{ij} " << MassMatrix.Label_Column << "_j";
    else
      (*this->out) << MassMatrix.Label_Row << "_i M_ij " << MassMatrix.Label_Column << "_j";
  }
  else
  {
    (*this->out) << "( ";
    for (i = 0; i < s1; ++i)
    {
      this->PrintLabel(Fields[MassMatrix.FieldIndices_Row[i]], VEVConfig.use_Labels);
      (*this->out) << " ";
    }
    (*this->out) << ")  M  (";
    ...
    for (i = 0; i < s2; ++i)
    {
      this->PrintLabel(Fields[MassMatrix.FieldIndices_Column[i]], VEVConfig.use_Labels);
      (*this->out) << " ";
    }
    (*this->out) << ")^T";
  }
  ...
  (*this->out) << "\t(" << setw(3) << s1 << " x" << setw(3) << s2 << " matrix)";
}



/* ########################################################################################
######   void CPrint::PrintMonomial(...)                                             ######
######                                                                               ######
######   Version: 25.10.2010                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) Monomial                      : CMonomial object to be printed           ######
######   2) VEVConfig                     : contains the fields (i.e. their labels)  ######
######   3) print_gauge_equivalent_fields : if true print equivalent fields in       ######
######                                      brackets {}                              ######
######   output:                                                                     ######
######   return value                     : length of output string                  ######
######################################################################################## */
unsigned CPrint::PrintMonomial(const CMonomial &Monomial, const SConfig &VEVConfig, bool print_gauge_equivalent_fields) const
{
  unsigned tmp = 0;
  string output = "";
  ...
  const size_t s1 = Monomial.GaugeEquivalentFields.size();
  for (unsigned i = 0; i < s1; ++i)
  {
    const vector<unsigned> &GaugeEquivalentFields_i = Monomial.GaugeEquivalentFields[i];

    tmp = Monomial.Exponents[i];

    if (print_gauge_equivalent_fields)
      print_max = GaugeEquivalentFields_i.size();
    else
      print_max = 1;

    if ((tmp != 1) || (print_max > 1))
      output += bracket1;

    if (print_gauge_equivalent_fields && (print_max > 1))
      output += set1;

    const CField &tmp_Field = VEVConfig.Fields[GaugeEquivalentFields_i[0]];
    std::ostringstream os1;
    os1 << tmp_Field.Numbers[VEVConfig.use_Labels];
    output += tmp_Field.Labels[VEVConfig.use_Labels] + "_" + number1 + os1.str() + number2;

    for (j = 1; j < print_max; ++j)
    {
      const CField &tmp_Field = VEVConfig.Fields[GaugeEquivalentFields_i[j]];
      std::ostringstream os1;
      os1 << tmp_Field.Numbers[VEVConfig.use_Labels];
      output += separator + tmp_Field.Labels[VEVConfig.use_Labels] + "_" + number1 + os1.str() + number2;
    }
    ...
    if (tmp != 1)
    {
      ...
      os2 << tmp;
      output += "^" + number1 + os2.str() + number2;
    }
...
{
  (*this->out) << "(" << Sector.Get_k() << ", " << Sector.Get_l() << ") with local twist: ";
  this->PrintTwist(Sector.GetTwist());
...
bool CPrint::PrintSGElement(const CSpaceGroupElement &SGElement, bool formatted) const
{
  if (this->output_type == Tlatex)
  {
    ...
    const bool lnotnull = (SGElement.Get_l() != 0);
    
    (*this->out) << "\\left(";
    if (knotnull || lnotnull)
    {
      if (knotnull)
      {
        (*this->out) << "\\theta";
      ...
      {
        (*this->out) << "\\omega";
        if (SGElement.Get_l() != 1)
          (*this->out) << "^{" << SGElement.Get_l() << "} ";
      }
    }
    else
      (*this->out) << "1";
  ...
  if (this->output_type == Tmathematica)
  {
    (*this->out) << "{" << SGElement.Get_k() << ", " << SGElement.Get_l() << ", ";
    for (unsigned i = 0; i < LatticeDim; ++i)
    {
...
{
  this->PrintVector(ShiftVector, ShiftVector.Lattice);
}
...
/* ########################################################################################
######   PrintNonLocalBreaking(const COrbifold &Orbifold) const                      ######
######                                                                               ######
######   Version: 30.04.2013                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) COrbifold  : COrbifold object to search for non-local breaking           ######
######   output:                                                                     ######
######   return value : is breaking non-local?                                       ######
######################################################################################## */
bool CPrint::PrintNonLocalBreaking(const COrbifold &Orbifold, const SConfig &VEVConfig, bool CheckVEVConfigGroupFromSimpleGUTGroup) const
{
  const double prec = 0.0001;
  ...
  const vector<COrbifoldGroupElement> &OElements = Orbifold.OrbifoldGroup.GetElements();
  size_t t1 = OElements.size();

  vector<CVector> AllLocalShifts;
  for (i = 0; i < t1; ++i)
  {
    const COrbifoldGroupElement &OElement = OElements[i];
    if (OElement.SGElement.WithTwist())
      AllLocalShifts.push_back(OElement.Shift);
  }
  ...
  {
    cout << "\n  Check the 10D gauge group. Return." << endl;
    return false;
  }
  ...
  bool unbroken = true;
  t1 = AllLocalShifts.size();
  for (i = 0; i < 480; ++i)
  {
    const CVector &Weight = Roots10D.Weights[i];
            
    unbroken = true;
    for (j = 0; unbroken && (j < t1); ++j)
    {
      if (!Weight.IsScalarProductInteger(AllLocalShifts[j]))
        unbroken = false;
    }
    if (unbroken)
  ...
  CGaugeGroup LocalGaugeGroup = determineAlgebra(Roots);

  const vector<gaugeGroupFactor<double> > copy_factors = LocalGaugeGroup.factor;
  const size_t s1 = copy_factors.size();

  if (Lattice == E8xE8)
  {
    LocalGaugeGroup.factor.clear();
    ...
    for (i = 0; i < s1; ++i)
    {
      ...
      {
        LocalGaugeGroup.factor.push_back(ggf);
        ++PosAnd;
      }
    }
    // from second E_8
    ...
    if (s1 != LocalGaugeGroup.factor.size())
    {
      cout << "\n  Warning in bool CPrint::PrintNonLocalBreaking(...) const: Cannot sort the gauge group factors. Number of factors before/after sorting: " << s1 << "/" << LocalGaugeGroup.factor.size() << ". Return false." << endl;
      return false;
    }
    ...
    {
      LocalGaugeGroup.algebra = "";
      for (i = 0; i < PosAnd; ++i)
      {
        LocalGaugeGroup.algebra += LocalGaugeGroup.factor[i].algebra;
        if (i + 1 < PosAnd)
          LocalGaugeGroup.algebra += " + ";
      }
      LocalGaugeGroup.algebra += " and ";
      for (i = PosAnd; i < s1; ++i)
      {
        LocalGaugeGroup.algebra += LocalGaugeGroup.factor[i].algebra;
        if (i + 1 <  copy_factors.size())
          LocalGaugeGroup.algebra += " + ";
      }
    }
    // end: compute the gauge group
  }
  
  if (LocalGaugeGroup.algebra == Orbifold.StandardConfig.SymmetryGroup.GaugeGroup.algebra)
    (*this->out) << "\n  " << this->cbegin << "Local breaking of gauge group:" << this->cend << "\n";
  else
    (*this->out) << "\n  " << this->cbegin << "Non-local breaking of gauge group:" << this->cend << "\n";

  (*this->out) << "  " << this->cbegin << "  local gauge group: " << LocalGaugeGroup.algebra << this->cend << "\n";
  (*this->out) << "  " << this->cbegin << "  4d gauge group   : " << Orbifold.StandardConfig.SymmetryGroup.GaugeGroup.algebra << this->cend << "\n" << endl;

  if (!CheckVEVConfigGroupFromSimpleGUTGroup)
    return true;


  size_t Total_Rank1 = 0;
  size_t Total_Rank2 = 0;

  t1 = LocalGaugeGroup.factor.size();
  for (i = 0; i < t1; ++i)
    Total_Rank1 += LocalGaugeGroup.factor[i].rank;

  t1 = Orbifold.StandardConfig.SymmetryGroup.GaugeGroup.factor.size();
  for (i = 0; i < t1; ++i)
    Total_Rank2 += Orbifold.StandardConfig.SymmetryGroup.GaugeGroup.factor[i].rank;

  if (Total_Rank1 == Total_Rank2)
  {
    (*this->out) << "  " << this->cbegin << "Ordinary local gauge group breaking." << this->cend << "\n" << endl;
    return false;
  }

  CAnalyseModel Analyse;
  const vector<unsigned> VEVConfig_observable_sector_GGs = VEVConfig.SymmetryGroup.observable_sector_GGs;
  const vector<unsigned> VEVConfig_observable_sector_U1s = VEVConfig.SymmetryGroup.observable_sector_U1s;

  vector<vector<double> > VEVConfigRootSpace;
      
  // begin: collect all simple roots and U(1) generators from the VEV config gauge group
  size_t t2 = VEVConfig_observable_sector_GGs.size();
  for (i = 0; i < t2; ++i)
  {
    const vector<vector<double> > &ggf_SimpleRoots = VEVConfig.SymmetryGroup.GaugeGroup.factor[VEVConfig_observable_sector_GGs[i]].simpleroots;
    VEVConfigRootSpace.insert(VEVConfigRootSpace.end(), ggf_SimpleRoots.begin(), ggf_SimpleRoots.end());
  }

  t2 = VEVConfig_observable_sector_U1s.size();
  for (i = 0; i < t2; ++i)
    VEVConfigRootSpace.push_back(VEVConfig.SymmetryGroup.GaugeGroup.u1directions[VEVConfig_observable_sector_U1s[i]]);
  // end: collect all simple roots and U(1) generators from the VEV config gauge group

  t2 = VEVConfigRootSpace.size();

  vector<vector<double> > OrthogonalSpace;
  bool FromThisLocalFactor = true;

  // run through the gauge group factors of the local group
  t1 = LocalGaugeGroup.factor.size();
  for (i = 0; i < t1; ++i)
  {
    ...
    if (!Find_Basis_Of_Orthogonal_Space(LocalGaugeGroup.factor[i].simpleroots, Lattice, 16, OrthogonalSpace))
    {
      cout << "\n  Warning in bool CPrint::PrintNonLocalBreaking(...) const: Could not find new basis for other U(1) directions. Return false." << endl;
      return false;
    }

    FromThisLocalFactor = true;
    for (j = 0; FromThisLocalFactor && (j < t2);++j)
    {
      const vector<double> &VEVConfigVector = VEVConfigRootSpace[j];
      for (k = 0; FromThisLocalFactor && (k < OrthogonalSpace.size());++k)
      {
        if (fabs(VEVConfigVector * OrthogonalSpace[k]) > prec)
          FromThisLocalFactor = false;
      }
    }
        
    if (FromThisLocalFactor)
    {
      if (VEVConfig.ConfigLabel == "StandardConfig")
        (*this->out) << "  " << this->cbegin << "  Gauge group in standard config.: ";
      else
        (*this->out) << "  " << this->cbegin << "  Gauge group in VEV config. \"" << VEVConfig.ConfigLabel << "\": ";
            
      t2 = VEVConfig_observable_sector_GGs.size();
      for (j = 0; j < t2; ++j)
      {
        this->PrintGaugeGroupFactor(VEVConfig, VEVConfig_observable_sector_GGs[j]);
        if (j+1 < t2)
        (*this->out) << " x ";
      }
      if (VEVConfig_observable_sector_U1s.size() != 0)
      {
        if (VEVConfig_observable_sector_U1s.size() == 1)
          (*this->out) << " x U(1)";
        else
          (*this->out) << " x U(1)^" << VEVConfig_observable_sector_U1s.size();
      }
            
      (*this->out) << " from simple local factor: " << LocalGaugeGroup.factor[i].algebra << "\n" << endl;
      return true;
    }
  }
  
  if (VEVConfig.ConfigLabel == "StandardConfig")
    (*this->out) << "  " << this->cbegin << "  Gauge group in standard config. not from simple local factor.\n" << endl;
  else
    (*this->out) << "  " << this->cbegin << "  Gauge group in VEV config. \"" << VEVConfig.ConfigLabel << "\" not from simple local factor.\n" << endl;
...
######   -                                                                           ######
######################################################################################## */
void CPrint::PrintState(const CState &State, const CSector &Sector, const CFixedBrane &FixedBrane, const SConfig &VEVConfig, const COrbifoldGroup &OrbifoldGroup) const
{
  unsigned i = 0;
  unsigned j = 0;
  ...
  (*this->out) << "=========================================================================\n";
  if ((SGElement.Get_k() == 0) && (SGElement.Get_l() == 0))
    (*this->out) << "  untwisted Sector\n";
  else
  {
    (*this->out) << "  (" << SGElement.Get_k() << ", " << SGElement.Get_l() << ") twisted Sector ";
  ...
  //CWilsonLine WL(E8xE8, 15.0/8.0,  3.0/8.0,  9.0/8.0,  -3.0/8.0,  -3.0/8.0,  9.0/8.0,  9.0/8.0,  9.0/8.0,  -3.0/8.0,  9.0/8.0, 15.0/8.0, 15.0/8.0,  3.0/8.0,  3.0/8.0,  3.0/8.0, 3.0/8.0);
  //CShiftVector LocalShift(E8xE8);
  //OrbifoldGroup.GetShiftVector(SGElement, LocalShift);
  ...
  else
  {
    ...
    for (i = 0; i < s1; ++i)
    {
      ...
      for (j = 0; j < s3; ++j)
      {
        ...
        //(*this->out) << " g_t = " << fmod(-1.0 * (WL * MLeftMover.Weights[Field.WeightIndices[j]]) + 0.5 * (WL * LocalShift),1.0) << " ";
...
bool CPrint::PrintStates(const COrbifold &Orbifold, const SConfig &VEVConfig) const
{
  ...
  for (i = 0; i < s1; ++i)
  {
    ...
    for (j = 0; j < s2; ++j)
    {
      ...
      for (k = 0; k < s3; ++k)
        this->PrintState(FixedBrane.GetInvariantState(k), Sector, FixedBrane, VEVConfig, OrbifoldGroup);
    }
  }
  (*this->out) << flush;
...
bool CPrint::PrintStates(const COrbifold &Orbifold, const SConfig &VEVConfig, const vector<unsigned> &FieldIndices, bool PrintInternalInformation) const
{
  ...
  for (i = 0; i < t1; ++i)
  {
    ...
    (*this->out) << "  sector (k,l)        : " << this->vector_open << Element.Get_k() << this->separator << Element.Get_l() << this->vector_close << "\n";
    ...
    if (Print_DiscreteNonRSymmetries)
    {
      ...
      for (j = 0; j < t2; ++j)
      {
        this->PrintRational(tmp_Field.GetDiscreteCharge(DiscreteNonRSymmetries[j], SpaceGroup, Orbifold.OrbifoldGroup), false);
        if (j+1 < t2)
          (*this->out) << this->separator;
      }
      (*this->out) << this->vector_close << "\n";
    ...
    if (t2 != 0)
    {
      ...
      for (j = 0; j < t2; ++j)
      {
        (*this->out) << tmp_Field.AccU1Charges[j];
        if (j+1 < t2)
          (*this->out) << this->separator;
      }
      (*this->out) << this->vector_close << "\n";
    ...
    (*this->out) << "\n";
    t2 = tmp_Field.GetNumberOfLMWeights();
    for (j = 0; j < t2; ++j)
    {
      this->PrintRational(tmp_Field.GetLMWeight(j, Sectors), Lattice, false);
      (*this->out) << "\n";
    }
    (*this->out) << "\n";
    (*this->out) << "  right-moving q_sh   : ";
    this->PrintRational(tmp_Field.q_sh, SO8, false);
    (*this->out) << "\n";
    ...
    if (Print_DiscreteRSymmetries)
    {
      ...
      for (j = 0; j < t2; ++j)
      {
        this->PrintRational(tmp_Field.GetDiscreteCharge(DiscreteRSymmetries[j], SpaceGroup, Orbifold.OrbifoldGroup), false);
        if (j+1 < t2)
          (*this->out) << this->separator;
      }
      (*this->out) << this->vector_close << "\n";
...
{
  if ((Sector.Get_k() == 0) && (Sector.Get_l() == 0))
    (*this->out) << "  " << this->cbegin << "U";
  else
    (*this->out) << "  " << this->cbegin << "T(" << Sector.Get_k() << "," << Sector.Get_l() << ")";
...
bool CPrint::PrintSummary(const CFixedBrane &FixedBrane, const COrbifoldGroup &OrbifoldGroup, const SConfig &VEVConfig, const SUSYMultiplet &Multiplet, bool print_Labels) const
{
  ...
  (*this->out) << "  -------------------------------------------------------------------------------------------------------------\n";
  (*this->out) << "  sector:      (k,l)  = (" << Element.Get_k() << ", " << Element.Get_l() << ")\n";
...
######   2) FieldCouplings     : the couplings to be printed, selected using         ######
######                           "FindCouplings(...)" from class "CAnalyseModel"     ######
######   3) EffectivePotential : replace fields by their vevs for non-vanishing vevs ######
######   4) PrintModuli        : print the moduli dependence on the moduli specified ######
######                           in the geometry file, i.e. T_i = T_ii i=1,2,3 and   ######
######                           U_i for i in a Z_2 plane                            ######
######   output:                                                                     ######
######   return value          : printed succesfully?                                ######
######################################################################################## */
bool CPrint::PrintCouplings(const SConfig &VEVConfig, const vector<YukawaCoupling> &FieldCouplings, bool EffectivePotential, bool PrintModuli) const
{
  string extra_space = "";
  ...
  {
    extra_space = "    ";
    (*this->out) << "W_" << this->set_open << "eff" << this->set_close << " =";
  }
  else
    (*this->out) << "W =";

  const size_t s1 = FieldCouplings.size();
  if (s1 == 0)
  {
    (*this->out) << " 0";
    if (this->output_type == Tmathematica)
  ...
  for (i = 0; i < s1; ++i)
  {
    const vector<unsigned> &FieldCoupling = FieldCouplings[i].FieldIndices;

    current_order = FieldCoupling.size();
    for (j = 0; j < current_order; ++j)
    {
      if (FieldCoupling[j] >= f1)
      {
        cout << "\n  Warning in bool CPrint::PrintCouplings(...) const: Field index out of range. Return false." << endl;
        return false;
      }
    }
  }
  ...
  if (EffectivePotential)
  {
    bool enter_printed = false;
    unsigned counter_terms_per_line = 0;
    vector<unsigned>                   tmp_FieldIndicesNoVEV;
    vector<unsigned>                   tmp_FieldIndicesVEV;
    vector<vector<unsigned> >          tmp_FieldIndicesOfVEVTerm;

    vector<vector<unsigned> >          FieldIndicesOfEffectiveTerms;
    vector<vector<vector<unsigned> > > FieldIndicesOfVEVTerms;

    vector<vector<string> >            VEVTermsLabelOfModuli;
    vector<vector<int> >               VEVTermsExponentsOfEtas;

    // begin: run through all couplings to be print
    for (i = 0; i < s1; ++i)
    {
      const YukawaCoupling   &Yukawa        = FieldCouplings[i];
      const vector<unsigned> &FieldCoupling = Yukawa.FieldIndices;

      current_order = FieldCoupling.size();

      // begin: sort the current coupling by the fields VEV
      tmp_FieldIndicesNoVEV.clear();
      tmp_FieldIndicesVEV.clear();
      for (j = 0; j < current_order; ++j)
      {
        const CField &Field = Fields[FieldCoupling[j]];
        if (fabs(Field.VEVs.GetLength()) < prec)
          tmp_FieldIndicesNoVEV.push_back(FieldCoupling[j]);
        else
          tmp_FieldIndicesVEV.push_back(FieldCoupling[j]);
      }
      // end: sort the current coupling by the fields VEV

      not_found = true;
      s2 = FieldIndicesOfEffectiveTerms.size();
      for (j = 0; not_found && (j < s2); ++j)
      {
        if (tmp_FieldIndicesNoVEV == FieldIndicesOfEffectiveTerms[j])
        {
          FieldIndicesOfVEVTerms[j].push_back(tmp_FieldIndicesVEV);
          
          VEVTermsLabelOfModuli.push_back(Yukawa.LabelOfModuli);
          VEVTermsExponentsOfEtas.push_back(Yukawa.ExponentsOfEtas);
      ...
      if (not_found)
      {
        FieldIndicesOfEffectiveTerms.push_back(tmp_FieldIndicesNoVEV);

        tmp_FieldIndicesOfVEVTerm.clear();
        tmp_FieldIndicesOfVEVTerm.push_back(tmp_FieldIndicesVEV);
        FieldIndicesOfVEVTerms.push_back(tmp_FieldIndicesOfVEVTerm);
      
        VEVTermsLabelOfModuli.push_back(Yukawa.LabelOfModuli);
        VEVTermsExponentsOfEtas.push_back(Yukawa.ExponentsOfEtas);
      }
      // end: run through all couplings to be print
    }
    // end: run through all couplings to be print
    ...
    for (i = 0; i < s2; ++i)
    {
      const vector<unsigned>          &FieldIndicesOfEffectiveTerm = FieldIndicesOfEffectiveTerms[i];
      const vector<vector<unsigned> > &FieldIndicesOfVEVTerm       = FieldIndicesOfVEVTerms[i];

      if (i != 0)
        (*this->out) << " +";

      // begin: print moduli if no terms with VEV
      s4 = FieldIndicesOfVEVTerm.size();
      if (PrintModuli && (s4 == 0))
      {
        s5 = VEVTermsLabelOfModuli[0].size();
        for (j = 0; j < s5; ++j)
        {
          if (VEVTermsExponentsOfEtas[0][j] != 0)
            (*this->out) << " eta(" << VEVTermsLabelOfModuli[0][j] << ")^" << VEVTermsExponentsOfEtas[0][j];
        }
      }
      // end: print moduli if no terms with VEV

      // begin: first, print the fields that have no VEV
      s3 = FieldIndicesOfEffectiveTerm.size();
      for (j = 0; j < s3; ++j)
      {
        (*this->out) << " ";
        this->PrintLabel(Fields[FieldIndicesOfEffectiveTerm[j]], VEVConfig.use_Labels);
      }
      // end: first, print the fields that have no VEV

      if (s4 == 0)
        ++counter_terms_per_line;
      else
      {
        if ((s4 != 1) || (FieldIndicesOfVEVTerm[0].size() != 0))
          (*this->out) << " ";

        // begin: then print the fields with VEV
        if ((s3 != 0) && (s4 > 1))
          (*this->out) << "(";
        ...
        for (j = 0; j < s4; ++j)
        {
          if (PrintModuli)
          {
            s5 = VEVTermsLabelOfModuli[j].size();
            for (k = 0; k < s5; ++k)
            {
              if (VEVTermsExponentsOfEtas[j][k] != 0)
                (*this->out) << " eta(" << VEVTermsLabelOfModuli[j][k] << ")^" << VEVTermsExponentsOfEtas[j][k];
            }
          }
          
          const vector<unsigned> &FieldIndices = FieldIndicesOfVEVTerm[j];

          s5 = FieldIndices.size();
          if (s5 == 0)
          {
            if (s4 > 1)
              (*this->out) << "1";
          ...
          else
          {
            for (k = 0; k < s5; ++k)
            {
              if (k != 0)
                (*this->out) << " ";
              (*this->out) << "<";
              this->PrintLabel(Fields[FieldIndices[k]], VEVConfig.use_Labels);
              (*this->out) << ">";
            }
          }
          // if there will be at least one more term
          if ((j+1) < s4)
          {
            ++counter_terms_per_line;
            // and the maximal number of terms per line is reached
            if (counter_terms_per_line % terms_per_line == 0)
            {
              // print new line
              counter_terms_per_line = 0;
              (*this->out) << "\n     " << extra_space;
            }
            (*this->out) << " + ";
          }
        ...
        // end: then print the fields with VEV
      }
      if (s4 > terms_per_line)
      {
        counter_terms_per_line = 0;
        (*this->out) << "\n   " << extra_space;
      }
  ...
  else
  {
    for (i = 0; i < s1; ++i)
    {
      ...
      if (PrintModuli)
      {
        s5 = Yukawa.LabelOfModuli.size();
        for (j = 0; j < s5; ++j)
        {
          if (Yukawa.ExponentsOfEtas[j] != 0)
            (*this->out) << " eta(" << Yukawa.LabelOfModuli[j] << ")^" << Yukawa.ExponentsOfEtas[j];
        }
      }
      ...
      {
        (*this->out) << " ";
        this->PrintLabel(Fields[FieldCoupling[j]], VEVConfig.use_Labels);
      }
      if ((i+1) < s1)
      {
...
/* ########################################################################################
######   PrintSuperpotential(const SConfig &VEVConfig, bool EffectivePotential) const######
######                                                                               ######
######   Version: 25.03.2011                                                         ######
######   Check-Level: 1                                                              ######
######                                                                               ######
###########################################################################################
######   input:                                                                      ######
######   1) VEVConfig          : contains all couplings to be printed and the fields ######
######   2) EffectivePotential : replace fields by their vevs for non-vanishing vevs ######
######   3) PrintModuli        : print eta(modulus) dependence                       ######
######   output:                                                                     ######
######   return value          : printed succesfully?                                ######
######################################################################################## */
bool CPrint::PrintSuperpotential(const SConfig &VEVConfig, bool EffectivePotential, bool PrintModuli) const
{
  return this->PrintCouplings(VEVConfig, VEVConfig.FieldCouplings, EffectivePotential, PrintModuli);
}
...
bool CPrint::TexSpectrum(const COrbifold &Orbifold, const SConfig &VEVConfig, const vector<unsigned> &FieldIndices, const vector<unsigned> &print_Labels)
{
  ...
  rational<int> RNormalization(0);
  vector<rational<int> > RNormalizations(ds, 1);
  rational<int> ThetaCharge(0);
  vector<rational<int> > ThetaCharges(ds, 1);

  unsigned i = 0;

  if (PrintRCharges)
  {
    // begin: find R normalization

    rational<int> Rcharge = 0;
    for (i = 0; i < ds; ++i)
    {
      const SDiscreteSymmetry &DiscreteSymmetry = DiscreteRSymmetries[i];
      ...
      for( vector<CField>::const_iterator it_field = Fields.begin(); it_field != Fields.end(); ++it_field)
      {
        ...
        {
          Rcharge = it_field->GetDiscreteCharge(DiscreteSymmetry,SpaceGroup, Orbifold.OrbifoldGroup);
          if (Rcharge != 0)
          {
            RNormalizations[i] = rational<int>(kgV(Rcharge.denominator(), RNormalizations[i].numerator()),1);

            Rcharge = it_field->GetDiscreteCharge(DiscreteSymmetry, SpaceGroup, Orbifold.OrbifoldGroup, false);
            if (Rcharge != 0)
              RNormalizations[i] = rational<int>(kgV(Rcharge.denominator(), RNormalizations[i].numerator()),1);
          }
        }
    ...
    for (i = 0; i < ds; ++i)
      ThetaCharges[i] = DiscreteRSymmetries[i].SuperpotentialCharge * RNormalizations[i]/2;
  }
  ...
  for (i = 0; i < p1; ++i)
  {
    tabular_parameter += "c";
    empty_line_string += " &";
    if ((VEVConfig.SymmetryGroup.observable_sector_U1s[i] == 0) && (VEVConfig.SymmetryGroup.IsFirstU1Anomalous))
            ...
            if (Field.SGElement.NoTwist())
            {
              ...
              if ((q_Vector == q_momentum_p) || (q_Vector == q_momentum_m))
              {
                ...
                if (PrintRCharges)
                {
                  ...
                  for (n = 0; n < ds; ++n)
                  {
                    this->PrintRational((Field.GetDiscreteCharge(DiscreteRSymmetries[n], SpaceGroup, Orbifold.OrbifoldGroup) * RNormalizations[n]) - ThetaCharges[n]);
                    if (n < ds-1)
                      (*this->out) << ", ";
                  }
                  (*this->out) << "$ ";
                ...
                {
                  (*this->out) << "& $";
                  if (Field.gamma_phases.size() == 9)
                    this->PrintRational(D2Rat(Field.gamma_phases[8]), false);
                  (*this->out) << "$ ";
                }
    ...
    else
    {
      ...
      for (j = 0; j < s2; ++j)
      {
        ...
        if (!fixedbrane_empty)
        {
          ...
          // begin: print localization (k, l; n_alpha)
          const CSpaceGroupElement &Label = FixedBrane.GetSGElement();
          (*this->out) << "$T_{(" << Label.Get_k() << ", " << Label.Get_l() << ")}^{(";
